...
const initialState: AuthState = {
  ...
  selectedTenant: localStorage.getItem('pos_tenant'),
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    setTenant(state, action: PayloadAction<string>) {
      ...
      localStorage.setItem('pos_tenant', action.payload);
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginUser.fulfilled, (state, action) => {
        ...
        // If demo user, auto set tenant to demo
        if (!state.selectedTenant) {
          state.selectedTenant = 'demo';
          localStorage.setItem('pos_tenant', 'demo');
        }
      })
...
export const { clearError, setTenant, addUser, updateUserInList, deleteUserFromList } = authSlice.actions;
